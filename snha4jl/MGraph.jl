...
function kroki(text="A --> B";filename=nothing,type="ditaa",ext="png",cache=true,name=nothing)
    ...
    url = replace(b64string, "+" => "-", "/" => "_", "=" => "")
    url = "https://kroki.io/$type/$ext/$url"
    ...
    if cache
        ...
        if !isfile(imgname)
            print("Downloading $(name === nothing ? "..." : name)...\n")
...
function main(argv)
    ...
    if length(argv) >= 1
        ...
        elseif argv[1] == "--gnew"
            ...
            if !(argv[2] in validtypes)
                ...
                if length(argv) == 2
                    ...
                    g = gnew(;type=argv[2], args...)
                    show(stdout, "text/plain", g)
                    println()
                    if v
                        g_dot = graph2dot(g, type=type, mode=args[:mode])
                        kroki(g_dot, type="graphviz")
                    end
                    if o
                        nrows = names(g,1)
                        ncols = names(g,2)
        ...
        elseif argv[1] == "--snha"
            ...
            close(f)
            data, header = readdlm(filename,',', header=true, skipstart=2)
            df = DataFrame(data, Symbol.(vec(header[1,1:end])))
            g_snha = asg(df; args...)
